...
- `GET /evals` returns summary-card metrics, raw `breach_counts`, field-level `field_accuracy`, explicit `missing_ground_truth` entries for requested fields not present in the fixtures, and prompt/model regression rows for the evals tab.
